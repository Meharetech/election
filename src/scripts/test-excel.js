...
async function runTest() {
  try {
    ...
    const mockData = [
      ['Campaign Target Groups', 'Contact Info', 'Notes'],
      ['https://www.facebook.com/groups/testgroup1', 'email@test.com', 'Primary target group'],
      ['Invalid Link (no URL)', '123-456-7890', 'Ignore me'],
      ['facebook.com/groups/no-protocol-link', 'admin@facebook.com', 'Should be parsed & prefixed with https://'],
      ['Some other random text', 'http://example.com/somepage', 'Matches general HTTP link']
    ];
    ...
    const tempFilePath = path.join(__dirname, 'mock_campaign_targets.xlsx');
    ...
    if (records.length > 0) {
      ...
      await GroupScraper.deleteMany({ uploadedBy: dummyUserId });
      const saved = await GroupScraper.insertMany(records);
      console.log(`✅ Successfully saved ${saved.length} records into Database:`);
      console.log(saved);

      // Verify fetch works
      const fetched = await GroupScraper.find({ uploadedBy: dummyUserId });
      console.log(`✅ Verified fetch from database. Found: ${fetched.length} links`);
    } else {
